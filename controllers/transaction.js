...
exports.postTransaction = async (req, res, next) => {
    ...
    switch (coinName) {
        ...
        case 'DGB':
            ...
            if (status == 1) {
                res.send(JSON.stringify({ status: "OK", coinName: coinName, addressNo: addressNo, addressTo: addressTo, value: value }))
            } else {
                res.send(JSON.stringify({ status: "Error", type: "4", reason: "Transaction error" }))
            }
            break
        case 'ETH':
